...
namespace EFCoreTest
{
    public class EDbContext : DbContext
    {
        ...
        {
            var entityEntries = ChangeTracker.Entries<TestConcurrency>().ToList();
            foreach (var entityEntry in entityEntries)
            {
                if (entityEntry.State == EntityState.Added)
                {
                    if (entityEntry.Entity.RowVersion == default)
                    {
                        entityEntry.Entity.RowVersion = DateTime.UtcNow.Ticks;
                    }
                }
